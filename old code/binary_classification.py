...
for split in ["train", "val"]:
    path = join("../datasets/VOCdevkit/VOC2011/ImageSets/Segmentation", split + ".txt")
    file_list = tuple(open(path, "r"))
    file_list = [id_.rstrip() for id_ in file_list]
    files[split] = file_list

for split in ["train", "val"]:
    binary_path = join("../datasets/VOCdevkit/VOC2011/ImageSets/Segmentation", split + "_binary.txt")
    ...
    if exists:
        ...
        with open(binary_path, 'a') as file:
            for ii in tqdm(files[split]):
                fname = ii + ".png"
                target_path = join("../datasets/VOCdevkit/VOC2011/SegmentationClass/pre_encoded", fname)
